 ...
 * Defaults match the original embed.js behaviour.
 *
 * Can coexist with the classic <script src="embed.js"> approach —
 * they are fully independent client instances sharing the same backend.
 */
(function () {
  'use strict';
  ...
  /* ── shared styles (injected once into document head) ───────────────── */
  var STYLE_ID = '__lc_wc_styles__';
  ...
  /* ══════════════════════════════════════════════════════════════════════
     LiveCursorsElement — <live-cursors> Custom Element
     ══════════════════════════════════════════════════════════════════════ */
  class LiveCursorsElement extends HTMLElement {
    ...
    constructor() {
      super();
      // Internal state — initialised in connectedCallback
      this._ws = null;
      this._selfId = null;
      this._selfUser = null;
    ...
    get containerSelector() { return this.getAttribute('container') || ''; }
    get presenceSelector() { return this.getAttribute('presence') || ''; }
    ...
    disconnectedCallback() {
      ...
      this._unbindEvents();
      this._closeWs();
      // Clean up DOM elements we appended outside this element
    ...
    _readToken() {
      ...
      try {
        ...
        if (fromUrl) {
          this._lcToken = fromUrl;
          localStorage.setItem(tokenKey, this._lcToken);
          up.delete('lc_token'); up.delete('token');
          var qs = up.toString();
          history.replaceState({}, '', location.pathname + (qs ? '?' + qs : '') + location.hash);
        } else {
          this._lcToken = localStorage.getItem(tokenKey) || null;
        }
        if (this._lcToken) {
          var p = JSON.parse(atob(this._lcToken.split('.')[1].replace(/-/g, '+').replace(/_/g, '/')));
          if (p && p.exp > Date.now() / 1000) {
            this._selfUser = { username: p.username, avatar: p.avatar, url: p.url };
          } else {
            this._lcToken = null; localStorage.removeItem(tokenKey);
          }
        }
      } catch (e) { this._lcToken = null; }
    }
    ...
    _handleVisibilityChange() { this._tabHidden = document.hidden; }
    ...
    _connect() {
      ...
      var wsUrl = proto + '//' + ORIGIN.replace(/^https?:\/\//, '') + '/ws?room=' + encodeURIComponent(this.room);
      ...
      ws.onclose = function (e) {
        ...
        if (self._connected) setTimeout(function () { self._connect(); }, self._reconnectDelay);
        self._reconnectDelay = Math.min(self._reconnectDelay * 1.5, 30000);
      };
    }

    _closeWs() { if (this._ws) { this._ws.close(); this._ws = null; } }
    ...
    _handle(m) {
      ...
      if (m.type === 'init') {
        ...
        // Derive selfColor from selfId
        if (this._selfId) {
          var hash = 0; for (var i = 0; i < this._selfId.length; i++) { hash = ((hash << 5) - hash) + this._selfId.charCodeAt(i); hash |= 0; }
          var colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4', '#FFEAA7', '#DDA0DD', '#98D8C8', '#F7DC6F', '#BB8FCE', '#85C1E9'];
          this._selfColor = colors[Math.abs(hash) % colors.length];
        }
        this._updatePresence();
      }
    ...
    _updatePresence() {
      ...
      if (this._selfUser) {
        ...
        var tk = this._tokenKey;
        logoutBtn.onclick = function () { localStorage.removeItem(tk); location.reload(); };
